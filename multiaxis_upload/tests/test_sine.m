...
for i=1:1
    ...
    walk = repmat([0.107821606112205	0.0930588023843044	0],10,1); %random(bounds(:,1:3), 1, 3);
    n = 1;
    for t = 1:size(walk,1)
        tarf(t,:) = 12.5 + 2 * (1+sin(linspace(1,2400,2400)/(220-20*t)));  %+ 2400/300      
    end
    flag = 2;
    ...
    for t = 1:size(walk, 1)
        ...
        for j = 1:2400
            ...
            elseif j == 30 % Rotate to new orientation
                wk = walk(t,:);
                mode = 1;
            elseif j <= 2400 % Enter control loop
                if get == 0
                    emp = [];
                    while length(emp) < 6
            ...
            if flag < 31
                target(flag) = tarf(t,1);
            else
                target(flag) = tarf(t,j-30);
            end
